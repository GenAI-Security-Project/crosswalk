...
- Entries renumbered: `LLM03→LLM04 · LLM04→LLM05 · LLM05→LLM10 · LLM06→LLM03 · LLM07→LLM08 · LLM08→LLM09 · LLM09→LLM07 · LLM10→LLM06`; LLM01 and LLM02 are unchanged
- All 23 `llm-top10/` framework files renumbered, reordered, and re-titled, with quick-reference tables, audience tags, and cross-references updated
- Cross-references to LLM entries updated throughout `agentic-top10/`, `dsgai-2026/`, `shared/`, `evals/`, `i18n/`, and the web app

#### Entries renamed and re-scoped
- **LLM08 Hidden Context Exposure** (was LLM07 System Prompt Leakage) — widened from the system prompt to all non-user-facing context: developer instructions, retrieved policy text, and tool and function schemas
- **LLM10 Improper Output Handling** (was LLM05 Insecure Output Handling) — widened to cover insecure code generated by coding assistants at scale
...
- **LLM05 Data and Model Poisoning** — scope note added for fine-tuning subversion via adapters, customer fine-tunes, and distilled checkpoints
...
- `evals/garak/` profiles renamed to the 2026 numbering: `LLM03_supply_chain` → `LLM04_supply_chain`, `LLM04_data_poisoning` → `LLM05_data_poisoning`, `LLM05_output_handling` → `LLM10_output_handling`, `LLM07_system_prompt_leakage` → `LLM08_hidden_context_exposure`, `LLM08_embeddings` → `LLM09_embeddings`, `LLM09_misinformation` → `LLM07_misinformation`, `LLM10_resource_exhaustion` → `LLM06_resource_exhaustion`
