...
Migration to the **OWASP Top 10 for LLM Applications 2026**. This is a breaking
change for anyone consuming LLM entry IDs: eight of the ten entries changed
number, and the change is a permutation, so a naive in-order find-and-replace
will corrupt references. See [MIGRATION.md](MIGRATION.md) for the full map.

### Changed

#### LLM Top 10 retargeted to the 2026 list

- `source_list` is now `LLM-Top10-2026` (was `LLM-Top10-2025`) across all ten LLM entries
- Crosswalk version bumped to `2026-Q3`
- Entries renumbered:
  `LLM03→LLM04 · LLM04→LLM05 · LLM05→LLM10 · LLM06→LLM03 · LLM07→LLM08 · LLM08→LLM09 · LLM09→LLM07 · LLM10→LLM06`
  ; LLM01 and LLM02 are unchanged
- All 23 `llm-top10/` framework files renumbered, reordered, and re-titled, with quick-reference tables,
  audience tags, and cross-references updated
- Cross-references to LLM entries updated throughout `agentic-top10/` , `dsgai-2026/` , `shared/` , `evals/`
  , `i18n/` , and the web app

#### Entries renamed and re-scoped

- **LLM08 Hidden Context Exposure** (was LLM07 System Prompt Leakage) — widened from the system prompt to
  all non-user-facing context: developer instructions, retrieved policy text, and tool and function schemas
- **LLM10 Improper Output Handling** (was LLM05 Insecure Output Handling) — widened to cover insecure code
  generated by coding assistants at scale
- **LLM04 Supply Chain** (was LLM03 Supply Chain Vulnerabilities) — renamed to match the 2026 title
- **LLM01 Prompt Injection** — scope note added for cross-modal payloads hidden in image, audio, and video content
- **LLM05 Data and Model Poisoning** — scope note added for fine-tuning subversion via adapters, customer
  fine-tunes, and distilled checkpoints

#### Severity re-baselining

- **LLM03 Excessive Agency** — High → **Critical** (climbed to third, the largest move on the 2026 list)
- **LLM06 Unbounded Consumption** — Medium → **High** (rose four places)
- **LLM07 Misinformation** — Medium → **High** (widest vote-versus-evidence gap in the 2026 incident corpus)
- Rationale recorded in [RATIONALE.md § 4.5](RATIONALE.md)

#### Eval profiles renumbered

- `evals/garak/` profiles renamed to the 2026 numbering: `LLM03_supply_chain` → `LLM04_supply_chain` ,
  `LLM04_data_poisoning` → `LLM05_data_poisoning` , `LLM05_output_handling` → `LLM10_output_handling` ,
  `LLM07_system_prompt_leakage` → `LLM08_hidden_context_exposure` , `LLM08_embeddings` → `LLM09_embeddings`
  , `LLM09_misinformation` → `LLM07_misinformation` , `LLM10_resource_exhaustion` →
  `LLM06_resource_exhaustion`
- `evals/ci/github-action.yml` and `evals/garak/run_all.sh` updated to the new paths

### Fixed

#### Stale pre-2025 entries in three framework files

`LLM_DORA.md`, `LLM_FedRAMP.md`, and `LLM_SP800218A.md` still carried entries
from the **2023** list that earlier updates had missed:

- Removed the duplicate `Model DoS` entry from each, which shadowed `Unbounded Consumption`
- Folded `Training Data Poisoning` into LLM05 Data and Model Poisoning
- Authored the missing LLM10 Improper Output Handling entry for DORA and FedRAMP
- Authored the missing LLM02 Sensitive Information Disclosure entry for SP 800-218A
- Their quick-reference tables were keyed to pre-2025 numbering; each file was remapped by entry name rather than by ID

### Added

- `MIGRATION.md` — 2025 → 2026 entry map, scope changes, severity basis, and a checklist for downstream consumers
- `RATIONALE.md § 4.5` — evidence basis for each severity change
- A 2026 migration row in the changelog table of every `llm-top10/` framework file
...
- 11 new framework registry files: OWASP NHI Top 10, PCI DSS v4.0, CWE/CVE, NIST SP 800-82 Rev 3, OWASP AI
  Testing Guide, NIST SP 800-218A, AIUC-1, FedRAMP, OWASP SAMM v2.0, ENISA Multilayer Framework, STRIDE
...
- `--format grc` : ServiceNow/Archer/Drata-ready JSON with control objectives, risk mappings, evidence
  requirements, and gap register
...
- 14 frameworks seeded: NIST AI RMF 1.0 (84 controls), EU AI Act CoP (54), ISO 27001 (41), SOC 2 (41), MITRE
  ATLAS (37), ISO 42001 (37), CoSAI (32), ISA/IEC 62443 (32), OWASP ASVS (32), MAESTRO (28), EU AI Act (25),
  NIST CSF 2.0 (22), DORA (22), CIS Controls (18) — **505 controls total**
...
- Review page (`#/review`): pending classifier PRs, local prediction review table, confidence slider,
...
- DeepSeek database exposure, o1/o3 CoT jailbreak, Cursor AI secrets, Garante GDPR enforcement, Clearview AI
  bias settlement, Azure OpenAI filter bypass, HuggingFace supply chain, NYT v OpenAI copyright, synthetic
  data re-identification, multi-agent trading cascade, Claude context flooding, adversarial RAG embeddings,
  Apollo Research scheming, AI companion exploitation, Stability AI CSAM, OpenAI GDPR erasure, Uber lineage
  audit, TikTok localization, Scale AI labeler exposure
...
17 new tools in TOOLS.md: Inspect AI, TextAttack, Counterfit, Foolbox, Mindgard, Agentic Security, OpenAI
Evals, Vigil, Arize Phoenix, AgentOps, LangSmith, Weave, OpenLLMetry, WhyLogs, Evidently, MLflow +
tools-supplement expanded to 19 entries

#### scripts/query.js — CLI query interface

Replaces jq with Node.js: `--stats` , `--severity` , `--framework` , `--entry` , `--tools` , `--incidents` ,
`--mappings` , `--framework-coverage` , `--incident-search` , `--aivss-above` . Output: table/json/csv.
...
Score measures control **depth** (not binary presence): severity-weighted average of controls covered /
total controls per entry.
...
- `src/index.ts` — typed API: `getEntry()` , `getFramework()` , `searchEntries()` , `getBySeverity()` ,
  `getIncidentsForEntry()` , `getIncidentsByLayer()`
...
LAAF v2.0 (<https://github.com/qorvexconsulting1/laaf-V2.0>) is the first automated red-teaming framework
purpose-built for Logic-layer Prompt Control Injection (LPCI) vulnerabilities in agentic LLM systems. This
release integrates it fully into the crosswalk evaluation suite.
...
- `evals/ci/github-action.yml` — `laaf-eval` job added (schedule + manual dispatch; 6 stages with crosswalk
  stage configs)
- `data/incidents.json` — INC-021: LAAF empirical study, 67–100% LPCI breakthrough rates across 5 production
  LLMs (Atta et al., arXiv:2507.10457)
...
`data/incidents.json` — 20 documented AI security incidents mapped to OWASP entries and MAESTRO
architectural layers (L1–L7). Each incident records where the attack originated, how it propagated, where
harm manifested, and where detection failed.
...
`data/incidents-schema.json` — JSON Schema (Draft 7) for incident entries. Each incident has: MAESTRO layer
attribution with roles (origin/propagation/impact/blind-spot), OWASP entry cross-references, attack vector,
impact, mitigations, and references.
...
- `scripts/generate.js` — now reads `data/incidents.json` at generation time and populates `incidents`
  arrays in all 41 entry JSON files
...
**Supported frameworks (17):** EU AI Act · ISO/IEC 42001:2023 · NIST AI RMF 1.0 · ISO/IEC 27001:2022 · SOC 2
· NIST CSF 2.0 · CIS Controls v8.1 · ISA/IEC 62443 · NIST SP 800-82 Rev 3 · OWASP ASVS · OWASP SAMM · PCI
DSS v4.0 · MITRE ATLAS · MAESTRO · AIUC-1 · ENISA Multilayer Framework · OWASP NHI Top 10
...
- `scripts/generate.js` — Markdown-to-JSON parser that reads all 58 mapping files and emits one structured
  JSON entry per vulnerability ID
...
- **License**: replaced incorrect MIT license text in `LICENSE.md` with CC BY-SA 4.0 — aligns with the
  license declared in `README.md` , all mapping file headers, and `CONTRIBUTING.md`
